...
class MetricsOutput(Outputable):
    ...
    def output_text(self):
        if (
            not self.metrics.eloc
            and not self.metrics.cyclomatic_complexity
        ...
        if self.metrics.cyclomatic_complexity_density:
            ...
            for i in sorted(
                set([(j, i) for (i, j) in self.metrics.cyclomatic_complexity_density.items()]), reverse=True
            ):
    ...
    def output_html(self):
        ...
        if self.metrics.cyclomatic_complexity_density:
            ...
            for num, i in enumerate(
                sorted(set([(j, i) for (i, j) in self.metrics.cyclomatic_complexity_density.items()]), reverse=True)
            ):
                metrics_xml += (
                    '<div class="'
