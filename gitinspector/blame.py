        ...
        if (
            ...
            if (self.blamechunk_time - self.changes.first_commit_date).days > 0:
                self.blames[(author, self.filename)].skew += (
                    self.changes.last_commit_date - self.blamechunk_time
                ).days / (7.0 if self.useweeks else AVG_DAYS_PER_MONTH)
...
class Blame(object):
    def __init__(self, repo, hard, useweeks, changes):
        ...
        ls_tree_p = subprocess.Popen(
            ["git", "ls-tree", "--name-only", "-r", interval.get_ref()],
