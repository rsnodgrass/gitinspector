...
class ChangesThread(threading.Thread):
    ...
    def run(self):
        ...
        for i in lines:
            ...
            if (
                FileDiff.is_filediff_line(j)
                and not filtering.set_filtered(FileDiff.get_filename(j))
                and not is_filtered
            ):
...
class Changes(object):
    ...
    def __init__(self, repo, hard):
        ...
        git_rev_list_p = subprocess.Popen(
            filter(
                None,
                ["git", "rev-list", "--reverse", "--no-merges", interval.get_since(), interval.get_until(), "HEAD"],
            ),
            stdout=subprocess.PIPE,
